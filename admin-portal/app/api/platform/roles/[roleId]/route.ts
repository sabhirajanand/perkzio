...
export async function GET(_req: Request, ctx: { params: Promise<{ roleId: string }> }) {
  ...
  const result = await proxyToBackend({
    method: 'GET',
    path: `/v1/platform/roles/${roleId}`,
    headers: await getAdminAuthHeader(),
  });
  if (!result.ok) return jsonError(result.status, 'Unable to fetch role');
...
export async function PATCH(req: Request, ctx: { params: Promise<{ roleId: string }> }) {
  ...
  if (!result.ok) return jsonError(result.status, 'Unable to update role');
  return NextResponse.json(result.json);
}

export async function DELETE(_req: Request, ctx: { params: Promise<{ roleId: string }> }) {
  const { roleId } = await ctx.params;
  const result = await proxyToBackend({
    method: 'DELETE',
    path: `/v1/platform/roles/${roleId}`,
  ...
  if (!result.ok) return jsonError(result.status, 'Unable to delete role');
  return NextResponse.json(result.json);
}
