...
export async function POST(_req: Request, ctx: { params: Promise<{ requestId: string }> }) {
  ...
  const result = await proxyToBackend({
    method: 'POST',
    path: `/v1/platform/merchant-branch-requests/${requestId}/approve`,
    headers: await getAdminAuthHeader(),
  });
  if (!result.ok) {
    const message =
      result.json && typeof result.json === 'object' && 'message' in (result.json as object)
        ? String((result.json as { message?: unknown }).message ?? 'Unable to approve branch request')
        : 'Unable to approve branch request';
