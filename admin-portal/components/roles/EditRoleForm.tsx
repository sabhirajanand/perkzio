...
import type { PlatformPermissionDto, RoleDetailDto } from '@/lib/platform/platformServer';
...
export default function EditRoleForm({ roleId, initial, permissions }: EditRoleFormProps) {
  const [name, setName] = useState(initial.role.name);
  const [description, setDescription] = useState(initial.role.description ?? '');
  const [selected, setSelected] = useState<Record<string, boolean>>(() => {
    const map: Record<string, boolean> = {};
    for (const code of initial.permissionCodes) map[code] = true;
    return map;
  });
